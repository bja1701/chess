...
public class PawnMovesCalculator implements PieceMovesCalculator{
    public Collection<ChessMove> pieceMoves(ChessBoard board,
        ...
        int color;
        if (board.getPiece(myPosition).getTeamColor()
        ...
        if (forwardPos.isOnBoard() && board.getPiece(forwardPos) == null
                && forwardPos.getRow() != promotionRow){
        ...
        } else if (forwardPos.getRow() == promotionRow
                && board.getPiece(forwardPos) == null){
            moves.add(new ChessMove(myPosition,
                    forwardPos, ChessPiece.PieceType.QUEEN));
            moves.add(new ChessMove(myPosition,
                    forwardPos, ChessPiece.PieceType.BISHOP));
            moves.add(new ChessMove(myPosition,
                    forwardPos, ChessPiece.PieceType.ROOK));
            moves.add(new ChessMove(myPosition,
                    forwardPos, ChessPiece.PieceType.KNIGHT));
        }
        ...
        for (int nextCol : captureCol){
            if (nextCol >= 1 && nextCol <= 8){
                ...
                if (enemyPiece != null && enemyPiece.getTeamColor()
                        != board.getPiece(myPosition).getTeamColor()){
                    if (enemyPos.getRow() == promotionRow){
                        moves.add(new ChessMove(myPosition,
                                enemyPos, ChessPiece.PieceType.QUEEN));
                        moves.add(new ChessMove(myPosition,
                                enemyPos, ChessPiece.PieceType.BISHOP));
                        moves.add(new ChessMove(myPosition,
                                enemyPos, ChessPiece.PieceType.ROOK));
                        moves.add(new ChessMove(myPosition,
                                enemyPos, ChessPiece.PieceType.KNIGHT));
                    } else {
